package com.pshs.ams.app.teachers.controllers;
...
import com.pshs.ams.app.teachers.models.dto.TeacherDTO;
...
public class TeacherController {
	...
	public Response getTeacherById(@PathParam("id") Long id) {
		if (id <= 0) {
			return Response.status(Response.Status.BAD_REQUEST).entity(new MessageResponse("Invalid id", CodeStatus.BAD_REQUEST))
					.build();
		}
		...
		return teacher.map(tch -> Response.ok(mapper.map(tch, TeacherDTO.class)).build()).orElseGet(() -> Response
				.status(Response.Status.NOT_FOUND).entity(new MessageResponse("Teacher not found", CodeStatus.NOT_FOUND)).build());
	}
	...
	public Response createTeacher(TeacherDTO teacherDTO) {
		if (teacherDTO == null) {
			return Response.status(Response.Status.BAD_REQUEST)
					.entity(new MessageResponse("Teacher cannot be null", CodeStatus.BAD_REQUEST)).build();
		}
	...
	public Response deleteTeacher(@PathParam("id") Integer id) {
		...
		return switch (teacherService.deleteTeacher(id)) {
			case OK -> Response.ok(new MessageResponse("Teacher deleted", CodeStatus.OK)).build();
			case BAD_REQUEST ->
				Response.status(Response.Status.BAD_REQUEST).entity(new MessageResponse("Invalid id", CodeStatus.BAD_REQUEST))
						.build();
			case NOT_FOUND ->
				Response.status(Response.Status.NOT_FOUND).entity(new MessageResponse("Teacher not found", CodeStatus.NOT_FOUND))
						.build();
			default ->
				Response.status(Response.Status.INTERNAL_SERVER_ERROR)
						.entity(new MessageResponse("Internal Server Error", CodeStatus.FAILED)).build();
		};
	}
	...
	public Response getTeacherByName(@PathParam("name") String name, @BeanParam PageRequest pageRequest,
		...
		if (name.isEmpty()) {
			return Response.status(Response.Status.BAD_REQUEST)
					.entity(new MessageResponse("Name cannot be empty", CodeStatus.BAD_REQUEST)).build();
		}
	...
	public Response updateTeacher(TeacherDTO teacherDTO, @PathParam("id") Long id) {
		if (teacherDTO == null || id <= 0) {
			return Response.status(Response.Status.BAD_REQUEST).entity(
					new MessageResponse("Teacher cannot be null or id cannot be less than or equal to zero", CodeStatus.BAD_REQUEST))
					.build();
		}
		...
		if (teacherOptional.isEmpty()) {
			return Response.status(Response.Status.NOT_FOUND)
					.entity(new MessageResponse("Teacher not found", CodeStatus.NOT_FOUND)).build();
		}

		Teacher teacher = mapper.map(teacherDTO, Teacher.class);
		return switch (teacherService.updateTeacher(teacher)) {
			case OK -> Response.ok(new MessageResponse("Teacher updated", CodeStatus.OK)).build();
			case BAD_REQUEST ->
