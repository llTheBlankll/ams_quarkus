...
public class UploadController {
	...
	public Response uploadTeacherProfilePicture(@PathParam("id") Long id, FileUploadInput files) {
		...
		// Validate each file
		for (FileUpload file : files.files) {
		...
		// Save the image file
		String secureFileName = UUID.randomUUID() + getFileExtension(files.files.get(0).fileName());
		File dest = new File("uploads", secureFileName);
		try (InputStream input = Files.newInputStream(files.files.get(0).filePath());
	...
	public Response uploadClassroomProfilePicture(@PathParam("id") Long id, FileUploadInput files) {
		...
		logger.debug("Files: " + (files.files != null ? files.files.size() : "null"));
